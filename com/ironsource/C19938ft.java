package com.ironsource;

import android.annotation.SuppressLint;
import android.os.Handler;
import android.webkit.RenderProcessGoneDetail;
import android.webkit.WebResourceError;
import android.webkit.WebResourceRequest;
import android.webkit.WebSettings;
import android.webkit.WebView;
import android.webkit.WebViewClient;
import com.ironsource.C19938ft;
import com.ironsource.C20540ns;
import com.ironsource.mediationsdk.C20423p;
import com.ironsource.mediationsdk.IronSource;
import com.ironsource.mediationsdk.testSuite.TestSuiteActivity;
import java.util.List;
import kotlin.Metadata;
import kotlin.collections.C37563v;
import kotlin.collections.CollectionsJVM;
import kotlin.jvm.internal.Intrinsics;
import kotlin.text.StringsJVM;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;
import org.json.JSONObject;
import p978t3.WebViewRenderException;

@Metadata
/* renamed from: com.ironsource.ft */
/* loaded from: classes6.dex */
public final class C19938ft implements InterfaceC20984ss {
    @NotNull

    /* renamed from: a */
    private final Handler f50140a;
    @Nullable

    /* renamed from: b */
    private WebView f50141b;
    @NotNull

    /* renamed from: c */
    private final WebView f50142c;
    @NotNull

    /* renamed from: d */
    private final JSONObject f50143d;

    /* renamed from: e */
    private boolean f50144e;

    /* renamed from: f */
    private boolean f50145f;
    @NotNull

    /* renamed from: g */
    private final InterfaceC20097ig f50146g;

    @Metadata
    /* renamed from: com.ironsource.ft$a */
    /* loaded from: classes6.dex */
    public static final class C19939a extends WebViewClient {

        /* renamed from: b */
        final /* synthetic */ String f50148b;

        C19939a(String str) {
            C19938ft.this = r1;
            this.f50148b = str;
        }

        @Override // android.webkit.WebViewClient
        public void onPageFinished(@Nullable WebView webView, @Nullable String str) {
            boolean m16624M;
            List<? extends Object> m17175e;
            if (!C19938ft.this.f50144e && str != null) {
                m16624M = StringsJVM.m16624M(str, String.valueOf(this.f50148b), false, 2, null);
                if (m16624M) {
                    C19938ft c19938ft = C19938ft.this;
                    m17175e = CollectionsJVM.m17175e(c19938ft.f50143d);
                    Intrinsics.m17073h(m17175e, "null cannot be cast to non-null type kotlin.collections.List<kotlin.Any?>");
                    c19938ft.m58511a(c19938ft.m58510a("onDataReady", m17175e));
                    C20605os.f52348a.m55901d();
                    C19938ft.this.f50144e = true;
                }
            }
            super.onPageFinished(webView, str);
        }

        @Override // android.webkit.WebViewClient
        public void onReceivedError(@NotNull WebView view, @NotNull WebResourceRequest request, @NotNull WebResourceError error) {
            Intrinsics.checkNotNullParameter(view, "view");
            Intrinsics.checkNotNullParameter(request, "request");
            Intrinsics.checkNotNullParameter(error, "error");
            C20605os.f52348a.m55904a(error.toString());
            C21333zs c21333zs = C21333zs.f54830a;
            c21333zs.m53716b("got error during TestSuite web controller loading " + error);
            super.onReceivedError(view, request, error);
        }

        @Override // android.webkit.WebViewClient
        public boolean onRenderProcessGone(WebView webView, RenderProcessGoneDetail renderProcessGoneDetail) {
            WebViewRenderException.m3275a(this);
            return true;
        }
    }

    @Metadata
    /* renamed from: com.ironsource.ft$b */
    /* loaded from: classes6.dex */
    public static final class C19940b implements InterfaceC20694qg {

        /* renamed from: b */
        final /* synthetic */ InterfaceC20694qg f50150b;

        C19940b(InterfaceC20694qg interfaceC20694qg) {
            C19938ft.this = r1;
            this.f50150b = interfaceC20694qg;
        }

        /* renamed from: a */
        public static final void m58499a(C19940b this$0) {
            Intrinsics.checkNotNullParameter(this$0, "this$0");
            this$0.onUIReady();
        }

        @Override // com.ironsource.InterfaceC20694qg
        public void onClosed() {
            this.f50150b.onClosed();
        }

        @Override // com.ironsource.InterfaceC20694qg
        public void onUIReady() {
            if (C19938ft.this.f50145f) {
                this.f50150b.onUIReady();
            } else {
                C19938ft.this.f50140a.postDelayed(new Runnable() { // from class: com.ironsource.dw
                    {
                        C19938ft.C19940b.this = this;
                    }

                    @Override // java.lang.Runnable
                    public final void run() {
                        C19938ft.C19940b.m58499a(C19938ft.C19940b.this);
                    }
                }, 500L);
            }
        }
    }

    public C19938ft(@NotNull TestSuiteActivity activity, @NotNull InterfaceC20694qg uiLifeCycleListener, @NotNull JSONObject jsonData, @Nullable String str) {
        Intrinsics.checkNotNullParameter(activity, "activity");
        Intrinsics.checkNotNullParameter(uiLifeCycleListener, "uiLifeCycleListener");
        Intrinsics.checkNotNullParameter(jsonData, "jsonData");
        this.f50140a = new Handler(activity.getMainLooper());
        this.f50141b = new WebView(activity);
        this.f50142c = new WebView(activity);
        this.f50143d = jsonData;
        this.f50146g = C19821el.f49880p.m58785d().mo57089r();
        m58513a(activity, uiLifeCycleListener, str);
    }

    /* renamed from: e */
    public static final void m58501e(C19938ft this$0) {
        Intrinsics.checkNotNullParameter(this$0, "this$0");
        this$0.f50145f = true;
    }

    @Nullable
    /* renamed from: d */
    public final WebView m58503d() {
        return this.f50141b;
    }

    @NotNull
    /* renamed from: a */
    public final String m58510a(@NotNull String methodName, @NotNull List<? extends Object> methodArgs) {
        Intrinsics.checkNotNullParameter(methodName, "methodName");
        Intrinsics.checkNotNullParameter(methodArgs, "methodArgs");
        String str = "TestSuiteJs." + methodName;
        String m58745b = C19853et.f49957a.m58745b(methodArgs);
        if (!(m58745b.length() > 0)) {
            return str + "()";
        }
        return str + '(' + m58745b + ')';
    }

    /* renamed from: b */
    public final void m58509b() {
        WebView webView = this.f50141b;
        if (webView != null) {
            webView.destroy();
            this.f50141b = null;
        }
    }

    @NotNull
    /* renamed from: c */
    public final WebView m58506c() {
        return this.f50142c;
    }

    /* renamed from: a */
    public final void m58517a() {
        m58509b();
        this.f50142c.destroy();
    }

    /* renamed from: a */
    public static final void m58515a(C19938ft this$0, String script) {
        Intrinsics.checkNotNullParameter(this$0, "this$0");
        Intrinsics.checkNotNullParameter(script, "$script");
        WebView webView = this$0.f50142c;
        webView.loadUrl("javascript:" + script);
    }

    @SuppressLint({"SetJavaScriptEnabled", "AddJavascriptInterface"})
    /* renamed from: a */
    private final void m58513a(TestSuiteActivity testSuiteActivity, final InterfaceC20694qg interfaceC20694qg, String str) {
        List<? extends IronSource.AD_UNIT> m17160s;
        WebView webView = this.f50141b;
        WebSettings settings = webView != null ? webView.getSettings() : null;
        boolean z = true;
        if (settings != null) {
            settings.setJavaScriptEnabled(true);
        }
        this.f50142c.getSettings().setJavaScriptEnabled(true);
        m17160s = C37563v.m17160s(IronSource.AD_UNIT.BANNER, IronSource.AD_UNIT.INTERSTITIAL);
        if (this.f50146g.mo57983a() != null) {
            List<IronSource.AD_UNIT> m56609p = C20423p.m56615m().m56609p();
            Intrinsics.checkNotNullExpressionValue(m56609p, "getInstance().legacyInitializedAdUnits");
            for (IronSource.AD_UNIT ad_unit : m56609p) {
                m17160s.remove(ad_unit);
            }
        } else {
            m17160s.clear();
        }
        this.f50142c.addJavascriptInterface(new C20775rs(new C20540ns.C20541a().m56094a(m17160s, testSuiteActivity, this.f50140a), new C19940b(interfaceC20694qg), this), "TestSuiteBridge");
        this.f50142c.setWebViewClient(new C19939a(str));
        if (str != null && str.length() != 0) {
            z = false;
        }
        if (!z) {
            this.f50142c.loadUrl(str);
        }
        WebView webView2 = this.f50141b;
        if (webView2 != null) {
            webView2.loadUrl("javascript:document.write(atob(''))");
        }
        this.f50140a.postDelayed(new Runnable() { // from class: com.ironsource.bw
            {
                C19938ft.this = this;
            }

            @Override // java.lang.Runnable
            public final void run() {
                C19938ft.m58501e(C19938ft.this);
            }
        }, 2000L);
        this.f50140a.postDelayed(new Runnable() { // from class: com.ironsource.cw
            @Override // java.lang.Runnable
            public final void run() {
                C19938ft.m58512a(interfaceC20694qg);
            }
        }, 5000L);
    }

    /* renamed from: a */
    public static final void m58512a(InterfaceC20694qg uiLifeCycleListener) {
        Intrinsics.checkNotNullParameter(uiLifeCycleListener, "$uiLifeCycleListener");
        uiLifeCycleListener.onUIReady();
    }

    /* renamed from: a */
    public final void m58511a(@NotNull final String script) {
        Intrinsics.checkNotNullParameter(script, "script");
        this.f50140a.post(new Runnable() { // from class: com.ironsource.aw
            {
                C19938ft.this = this;
            }

            @Override // java.lang.Runnable
            public final void run() {
                C19938ft.m58515a(C19938ft.this, script);
            }
        });
    }

    @Override // com.ironsource.InterfaceC20984ss
    /* renamed from: a */
    public void mo54892a(@NotNull String callbackName, @NotNull IronSource.AD_UNIT adUnit, @NotNull List<? extends Object> args) {
        List<? extends Object> m17163p;
        Intrinsics.checkNotNullParameter(callbackName, "callbackName");
        Intrinsics.checkNotNullParameter(adUnit, "adUnit");
        Intrinsics.checkNotNullParameter(args, "args");
        if (this.f50144e) {
            String ad_unit = adUnit.toString();
            Intrinsics.checkNotNullExpressionValue(ad_unit, "adUnit.toString()");
            m17163p = C37563v.m17163p(callbackName, ad_unit, args);
            m58511a(m58510a("onCallbackReceived", m17163p));
        }
    }
}
