package com.zeus.gmc.sdk.mobileads.columbus.p626ad.mraid;

import android.content.Context;
import android.graphics.Bitmap;
import android.graphics.BitmapFactory;
import android.graphics.drawable.BitmapDrawable;
import android.graphics.drawable.Drawable;
import android.util.Base64;
import com.zeus.gmc.sdk.mobileads.columbus.internal.coiio2.C32726coi222o222;

/* renamed from: com.zeus.gmc.sdk.mobileads.columbus.ad.mraid.Drawables */
/* loaded from: classes8.dex */
public enum Drawables {
    INTERSTITIAL_CLOSE_BUTTON_NORMAL(""),
    INTERSTITIAL_CLOSE_BUTTON_PRESSED("");
    
    private Bitmap cachedBitmap;
    private final String encodeString;

    Drawables(String str) {
        this.encodeString = str;
    }

    public void clear() {
        this.cachedBitmap = null;
    }

    public Drawable createDrawable(Context context) {
        BitmapDrawable bitmapDrawable = new BitmapDrawable(context.getResources(), getBitmap());
        bitmapDrawable.setTargetDensity(C32726coi222o222.coi222o222(context.getResources().getDisplayMetrics().xdpi, context));
        return bitmapDrawable;
    }

    public Bitmap getBitmap() {
        if (this.cachedBitmap == null) {
            byte[] decode = Base64.decode(this.encodeString, 0);
            this.cachedBitmap = BitmapFactory.decodeByteArray(decode, 0, decode.length);
        }
        return this.cachedBitmap;
    }
}
