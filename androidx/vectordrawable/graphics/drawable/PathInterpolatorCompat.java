...
public class PathInterpolatorCompat implements Interpolator {
    ...
    /* renamed from: mX */
    private float[] f1474mX;

    /* renamed from: mY */
    private float[] f1475mY;
    ...
    private void initPath(Path path) {
        ...
        if (min > 0) {
            this.f1474mX = new float[min];
            this.f1475mY = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((i2 * length) / (min - 1), fArr, null);
                this.f1474mX[i2] = fArr[0];
                this.f1475mY[i2] = fArr[1];
            }
            if (Math.abs(this.f1474mX[0]) <= 1.0E-5d && Math.abs(this.f1475mY[0]) <= 1.0E-5d) {
                int i3 = min - 1;
                if (Math.abs(this.f1474mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f1475mY[i3] - 1.0f) <= 1.0E-5d) {
                    float f = 0.0f;
                    int i4 = 0;
                    while (i < min) {
                        float[] fArr2 = this.f1474mX;
                        int i5 = i4 + 1;
                        float f2 = fArr2[i4];
                        if (f2 >= f) {
                            fArr2[i] = f2;
            ...
            sb2.append(this.f1474mX[0]);
            sb2.append(",");
            sb2.append(this.f1475mY[0]);
            sb2.append(" end:");
            int i6 = min - 1;
            sb2.append(this.f1474mX[i6]);
            sb2.append(",");
            sb2.append(this.f1475mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int length = this.f1474mX.length - 1;
        int i = 0;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f1474mX[i2]) {
                length = i2;
            } else {
                i = i2;
            }
        }
        float[] fArr = this.f1474mX;
        float f2 = fArr[length];
        float f3 = fArr[i];
        float f4 = f2 - f3;
        if (f4 == 0.0f) {
            return this.f1475mY[i];
        }
        float[] fArr2 = this.f1475mY;
        float f5 = fArr2[i];
