...
public final class Id3Decoder extends SimpleMetadataDecoder {
    ...
    /* JADX WARN: Code restructure failed: missing block: B:308:0x0194, code lost:
        if (r13 == 67) goto L98;
     */
    /* JADX WARN: Removed duplicated region for block: B:340:0x0209  */
    ...
    */
    ...
    private static Id3Header decodeHeader(ParsableByteArray parsableByteArray) {
        ...
        if (parsableByteArray.bytesLeft() < 10) {
            Log.m104564w(TAG, "Data too short to be an ID3 tag");
            return null;
        }
        boolean z5 = false;
        if (parsableByteArray.readUnsignedInt24() != 4801587) {
            Log.m104564w(TAG, "Unexpected first three bytes of ID3 tag header: 0x" + String.format("%06X", Integer.valueOf(readUnsignedInt24)));
            return null;
        }
        int readUnsignedByte = parsableByteArray.readUnsignedByte();
        ...
        if (readUnsignedByte == 2) {
            ...
            if (z4) {
                Log.m104564w(TAG, "Skipped ID3 tag with majorVersion=2 and undefined compression scheme");
                return null;
            }
            ...
            Log.m104564w(TAG, "Skipped ID3 tag with unsupported majorVersion=" + readUnsignedByte);
            return null;
        }
    ...
    public Metadata decode(byte[] bArr, int i) {
        ...
        if (!validateFrames(parsableByteArray, decodeHeader.majorVersion, i2, false)) {
            if (decodeHeader.majorVersion != 4 || !validateFrames(parsableByteArray, 4, i2, true)) {
                Log.m104564w(TAG, "Failed to validate ID3 tag with majorVersion=" + decodeHeader.majorVersion);
                return null;
            }
            z = true;
        }
