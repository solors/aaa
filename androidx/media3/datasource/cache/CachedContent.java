...
public final class CachedContent {
    ...
    public boolean equals(@Nullable Object obj) {
        ...
        if (this.f861id == cachedContent.f861id && this.key.equals(cachedContent.key) && this.cachedSpans.equals(cachedContent.cachedSpans) && this.metadata.equals(cachedContent.metadata)) {
            return true;
        }
        return false;
    }
    ...
    public int hashCode() {
        return (((this.f861id * 31) + this.key.hashCode()) * 31) + this.metadata.hashCode();
    }
    ...
    public SimpleCacheSpan setLastTouchTimestamp(SimpleCacheSpan simpleCacheSpan, long j, boolean z) {
        ...
        if (z) {
            File cacheFile = SimpleCacheSpan.getCacheFile((File) Assertions.checkNotNull(file.getParentFile()), this.f861id, simpleCacheSpan.position, j);
            if (file.renameTo(cacheFile)) {
                file = cacheFile;
            } else {
                Log.m104564w(TAG, "Failed to rename " + file + " to " + cacheFile);
            }
        }
