...
public class FlexBuffersBuilder {
    ...
    /* renamed from: bb */
    private final ReadWriteBuf f696bb;
    ...
    private int align(int i) {
        int i2 = 1 << i;
        int paddingBytes = Value.paddingBytes(this.f696bb.writePosition(), i2);
        while (true) {
            int i3 = paddingBytes - 1;
            if (paddingBytes != 0) {
                this.f696bb.put((byte) 0);
                paddingBytes = i3;
            } else {
                return i2;
            }
    ...
    private Value createKeyVector(int i, int i2) {
        ...
        while (i3 < this.stack.size()) {
            i3++;
            max = Math.max(max, Value.elemWidth(4, 0, this.stack.get(i3).key, this.f696bb.writePosition(), i3));
        }
        int align = align(max);
        writeInt(j, align);
        int writePosition = this.f696bb.writePosition();
    ...
    private Value createVector(int i, int i2, int i3, boolean z, boolean z2, Value value) {
        ...
        int max = Math.max(0, widthUInBits(j));
        if (value != null) {
            max = Math.max(max, value.elemWidth(this.f696bb.writePosition(), 0));
            i4 = 3;
        } else {
            i4 = 1;
        }
        ...
        for (int i9 = i2; i9 < this.stack.size(); i9++) {
            i8 = Math.max(i8, this.stack.get(i9).elemWidth(this.f696bb.writePosition(), i9 + i4));
            if (z && i9 == i2) {
        ...
        if (!z) {
            while (i10 < this.stack.size()) {
                this.f696bb.put(this.stack.get(i10).storedPackedType(i8));
                i10++;
            }
        }
        if (value != null) {
    ...
    private int putKey(String str) {
        ...
        if ((this.flags & 1) != 0) {
            ...
            if (num == null) {
                ...
                this.f696bb.put(bytes, 0, bytes.length);
                this.f696bb.put((byte) 0);
        ...
        this.f696bb.put(bytes2, 0, bytes2.length);
        this.f696bb.put((byte) 0);
        this.keyPool.put(str, Integer.valueOf(writePosition));
        return writePosition;
    }
    ...
    private Value writeBlob(int i, byte[] bArr, int i2, boolean z) {
        ...
        int writePosition = this.f696bb.writePosition();
        this.f696bb.put(bArr, 0, bArr.length);
        if (z) {
            this.f696bb.put((byte) 0);
        }
    ...
    private void writeDouble(double d, int i) {
        if (i == 4) {
            this.f696bb.putFloat((float) d);
        } else if (i == 8) {
            this.f696bb.putDouble(d);
        }
    }
    ...
    private void writeInt(long j, int i) {
        if (i != 1) {
            if (i != 2) {
                if (i != 4) {
                    if (i == 8) {
                        this.f696bb.putLong(j);
                        return;
                    }
                    return;
                }
                this.f696bb.putInt((int) j);
                return;
            }
            this.f696bb.putShort((short) j);
            return;
        }
        this.f696bb.put((byte) j);
    }

    private void writeOffset(long j, int i) {
        writeInt((int) (this.f696bb.writePosition() - j), i);
    }
    ...
    public ByteBuffer finish() {
        int align = align(this.stack.get(0).elemWidth(this.f696bb.writePosition(), 0));
        writeAny(this.stack.get(0), align);
        this.f696bb.put(this.stack.get(0).storedPackedType());
        this.f696bb.put((byte) align);
        this.finished = true;
        return ByteBuffer.wrap(this.f696bb.data(), 0, this.f696bb.writePosition());
    }
    ...
    public FlexBuffersBuilder(ReadWriteBuf readWriteBuf, int i) {
        ...
        this.keyComparator = new Comparator<Value>() { // from class: androidx.emoji2.text.flatbuffer.FlexBuffersBuilder.1
            ...
            public int compare(Value value, Value value2) {
                ...
                int i3 = value2.key;
                do {
                    b = FlexBuffersBuilder.this.f696bb.get(i2);
                    b2 = FlexBuffersBuilder.this.f696bb.get(i3);
                    if (b == 0) {
                        return b - b2;
                    }
                    i2++;
